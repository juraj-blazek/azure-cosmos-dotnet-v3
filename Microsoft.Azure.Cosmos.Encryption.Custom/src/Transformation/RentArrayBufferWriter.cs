...
/// Workaround for netstandard2.0 absence of ArrayBufferWriter
/// </summary>
internal class RentArrayBufferWriter : IBufferWriter<byte>, IDisposable
{
    ...
    public async Task CopyToAsync(Stream stream, CancellationToken cancellationToken = default)
    {
        ...
        await stream.WriteAsync(this.rentedBuffer, 0, this.written, cancellationToken).ConfigureAwait(false);
    ...
    private void CheckIfDisposed()
    {
        if (this.rentedBuffer == null)
        {
            throw new ObjectDisposedException(nameof(RentArrayBufferWriter));
        }
    }
